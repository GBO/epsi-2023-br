...
Abdoulaye::Abdoulaye() : FighterBot("Abdoulaye", 2, 28, 0) {
    this->targetId = "";
}

Fighter Abdoulaye::selectTarget(Arena arena) {
    vector<Fighter> fighters = arena.get();
    Fighter target = this->getFighter();

    // Pas de cible retenu, on en choisit une !
    if (this->targetId == "") {
        // Reste-t-il autre chose que des Abdoulaye dans l'arene ?
        // Parce que bon, on va d'abord viser les autres avant de se taper dessus ^_^'
        bool onlyAbdoulaye = true;
        for (Fighter fighter : fighters) {
            if (fighter.getName() != "Abdoulaye") {
                onlyAbdoulaye = false;
                break;
            }
        }
        ...
        if (onlyAbdoulaye) {
            ...
            while (this->isMe(target) || (target.getName() == "Abdoulaye")) {
                target = fighters[rand() % fighters.size()];
...
Action* Abdoulaye::choose(Arena arena) {
